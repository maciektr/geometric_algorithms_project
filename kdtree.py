...
class Node:
    # Konstruktor klasy
    def __init__(self, point=None, line=None, left=None, right=None, visualiser=None):
    ...
    def check_child(self, child, actual_scope, depth, scope):
        if child is not None:
            ...
            if self.visualiser is not None:
                self.visualiser.add_search(new_scope)

            if scope.contains(new_scope):
                result = child.report_subtree()
                if self.visualiser is not None:
                    self.visualiser.add_search(new_scope, [i.get_tuple() for i in result] if result is not None else None)
                return result
            elif scope.intersects(new_scope):
                result = child._search(scope, new_scope, depth + 1)
                if self.visualiser is not None:
                    self.visualiser.add_search(new_scope, [i.get_tuple() for i in result] if result is not None else None)
                return result
            return None
...
class Kdtree:
    ...
    def __init__(self, points, visualiser=None):
        self.visualiser = visualiser
        if self.visualiser is not None:
            self.visualiser.put_points(points)
    ...
    def _construct(self, points, depth=0):
        ...
        if self.visualiser is not None:
            if depth %2 == 0:
                y_l = min(points, key=lambda n:n[1])[1]
                y_h = max(points, key=lambda n:n[1])[1]
                self.visualiser.add_line([(m,y_l),(m, y_h)], points)
            else:
                x_l = min(points, key=lambda n: n[0])[0]
                x_h = max(points, key=lambda n: n[0])[0]
                self.visualiser.add_line([(x_l, m), (x_h, m)], points)
    ...
    def find(self,  x_low=-np.inf, x_high=np.inf, y_low=-np.inf, y_high=np.inf):
        ...
        if self.visualiser is not None:
            self.visualiser.add_search(self.scope)

        res = [p.get_tuple() for p in self.root._search(scope, self.scope)]

        if self.visualiser is not None:
            self.visualiser.add_search(self.scope, res)
